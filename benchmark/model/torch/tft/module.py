...
class TemporalFusionTransformerModel(nn.Module):
    ...
    def __init__(
        ...
        if self.d_past_feat_dynamic_real:
            self.past_feat_dynamic_proj = FeatureProjector(
                ...
                embedding_dims=[self.d_var] * len(self.d_past_feat_dynamic_real),
            )
        else:
        ...
        if self.c_past_feat_dynamic_cat:
            self.past_feat_dynamic_embed = FeatureEmbedder(
                ...
                embedding_dims=[self.d_var] * len(self.c_past_feat_dynamic_cat),
            )
        else:
    ...
    def forward(
        ...
        ctx_input, _ = self.ctx_selector(past_covariates, c_selection)  # [N, T, d_var]
