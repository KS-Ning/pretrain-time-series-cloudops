...
from .lightning_module import DeepVARLightningModule
...
class DeepVAREstimator(PyTorchLightningEstimator):
    """
    Estimator class to train a DeepVAR model, as described in [SFG17]_.
    This class is uses the model defined in ``DeepVARModel``, and wraps it
    into a ``DeepVARLightningModule`` for training purposes: training is
    ...
    *Note:* the code of this model is unrelated to the implementation behind
    `SageMaker's DeepVAR Forecasting Algorithm
    <https://docs.aws.amazon.com/sagemaker/latest/dg/DeepVAR.html>`_.
    Parameters
    ----------
    freq
        ...
        return Chain(
            ...
            + [
                ...
                AsNumpyArray(
                    ...
                    expected_ndim=1 + self.distr_output.event_dim,
                ),
                AddObservedValuesIndicator(
    ...
    def _create_instance_splitter(self, module: DeepVARLightningModule, mode: str):
    ...
    def create_training_data_loader(
        self,
        data: Dataset,
        module: DeepVARLightningModule,
        shuffle_buffer_length: Optional[int] = None,
        **kwargs,
    ) -> Iterable:
        transformation = self._create_instance_splitter(
            module, "training"
        ) + SelectFields(TRAINING_INPUT_NAMES, allow_missing=False)
        training_instances = transformation.apply(Cyclic(data))
    ...
    def create_lightning_module(self) -> DeepVARLightningModule:
        model_kwargs = {
            "freq": self.freq,
    ...
    def create_predictor(
        ...
        prediction_splitter = self._create_instance_splitter(
            module, "test"
        ) + RemoveFields([FieldName.PAST_FEAT_DYNAMIC_REAL])
